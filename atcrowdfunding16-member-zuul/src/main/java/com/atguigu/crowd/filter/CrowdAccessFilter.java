...
public class CrowdAccessFilter extends ZuulFilter {
	...
	public boolean shouldFilter() {
		...
		// 4.根据servletPath判断当前请求是否对应可以直接放行的特定请求地址
		boolean containsResult = AccessPassResources.PASS_RES_SET.contains(servletPath);

		// 5.如果当前请求是可以直接放行的特定请求地址则返回false放行
		if (containsResult) {
			return false;
		}

		// 6.判断当前请求是否为静态资源
		// 工具方法返回true：说明当前请求是静态资源请求，取反为false表示放行，不做登录检查
		// 工具方法返回false：说明当前请求不是静态资源请求，取反为true表示需要做登录检查
